...
#define Max_conf         5   /*число конфиг.параметров задачи*/
#define Max_select       9
#define Max_dyn          10
#define Max_dyn_massive 21
#define Max_main        68   /*число конфиг.параметров станции*/
...
const unsigned char size_conf[8]={21,21,21,21,240,0,216,237};
const unsigned char etl[8] = {1,2,4,8,16,32,64,128};
const unsigned char ind_prm_alm[4]={81,82,84,85};
const unsigned char typ_port[6] = {3,5,1,4,4,4};
...
const unsigned char main_dyn[Max_dyn][3]={
 ...
 {28,1,0}/*Тотал обм от масс*/,{29,4,3}};
...
{1,0,5,24,0,0,194,1,0}/*Мгн расход массы от мм*/,
{10,0,6,4,0,0,194,1,0}/*Мгн расход обм от мм*/,
{2,0,49,8,0,0,194,1,0}/*Плотность от мм*/,
{6,0,3,3,0,0,194,1,0}/*Температ от мм*/,
{27,0,67,13,0,0,194,1,0}/*Масса тотал от мм*/,
{28,0,68,14,0,0,194,1,0}/*Объём тотал от мм*/,
{29,0,4,8,0,0,194,1,0},
{11,1,5,26,2,0,194,1,1}/*Масса за час*/,
{11,2,5,26,2,0,194,1,1}/*Масса за сутки*/,
{11,3,5,26,2,0,194,1,1}/*Масса за пр сутки*/,
{11,4,5,26,2,0,194,1,1}/*Масса за месяц*/,
{11,5,5,26,2,0,194,1,1}/*Масса за пр мес*/,
{11,6,5,26,2,0,194,1,1}/*Масса за год*/,
{19,0,6,17,2,0,194,1,1}/**/,
{19,1,6,18,2,0,194,1,1}/*Объём за час*/,
{19,2,6,18,2,0,194,1,1}/*Объём за сутки*/,
{19,3,6,18,2,0,194,1,1}/*Объём за пр сутки*/,
{19,4,6,18,2,0,194,1,1}/*Объём за месяц*/,
{19,5,6,18,2,0,194,1,1}/*Объём за пр месяц*/,
{19,6,6,18,2,0,194,1,1}/*Объём за год*/,
...
struct station
{
  ...
  unsigned char com[6][2];               /*настройки портов*/
...
void InitStationStruct (struct station *dv)
{
  ...
  for (i=0;i<2;i++)
  {dv->com[i+4][0]=ReadEEP(7,232+i*2);dv->com[i+4][1]=ReadEEP(7,232+i*2);}
}
		      ...
		      struct dynparams *bs)
{
  ...
  if (SecurityConvert(ReadEEP(num,2),ReadEEP(num,3),
       ReadEEP(num,4),ReadEEP(num,5),cr)==0)
  cnf->cut_flow=ConvToFloat(cr[0],cr[1],cr[2],cr[3]);
  for (i=0;i<10;i++) cnf->tag[i]=ReadEEP(num,6+i);
  cnf->polar=ReadEEP(num,16);
  if (SecurityConvert(ReadEEP(num,17),ReadEEP(num,18),
       ReadEEP(num,19),ReadEEP(num,20),cr)==0)
  cnf->density=ConvToFloat(cr[0],cr[1],cr[2],cr[3]);
  bs->dyn[29]=cnf->density;
}
