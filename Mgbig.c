...
//01.05.2020 YN -----\\//-----
#define Typ_task          2  //was:8  /*учёт топлива*/
//------------- -----//\\-----
...
struct runtime             Prt;
const unsigned char ind_port[3] = {0,1,3};
...
unsigned char meter_func, pool_slv;
...
float proba[2];/*unsigned char proba[15];*/
/*unsigned count_tick;
/*объявления функций*/
...
void SendToSlaveController (unsigned char evt);
void ReceiveFromSlaveContr(unsigned char buf[], unsigned char *evt);
/*****************************************************************/
/*void ABC (void)
{
...
void main (void)
{
  ...
  InitializeMain();ReinstallPort(1);ReinstallPort(2);
  InstallCom_3(38400L,8,0,1); Port[2].ta=3;
 ...
 /* cr[3]=255;cr[2]=128;cr[1]=0;cr[0]=0; ff=1;
  ...
  {
    /*printf(" begin ");
    printf(" %f ",Config[0].density);
    printf(" %f ",Basic[0].dyn[29]);*/
    /*printf(" %d ",Config[0].status);*//*printf(" %d ",flg_zero_meter);*/
  ...
  //------------- -----//\\-----
	...
	} else if (icp_pool == 18) {ReadPageMMI(Port[0].buf); Display.evt=0;}
    ...
    {
      flg_min=0; Port[2].evt=7;/* обновление инициализации датчиков */
...
void MyTimer (void)
{
   ...
   if (pool_slv != 0)
   {
     ...
     {
       Port[2].timer=Port[2].index=Port[2].status=0;/*printf("err%d ",typ_pool);*/
       pool_slv=0; ClearCom(3);
     }
     if (Port[2].status==1)
     {
       if (Port[2].index > 7) if (Port[2].index >= Port[2].buf[6])
       {
	 Port[2].status=2; Port[2].timer=0; Port[2].index=a; ClearCom_3();
       }
     }
   }
}
...
void InitializeMain (void)
{
  ...
  for (i=0;i<Max_pnt;i++) InitBasicStruct(i,&Config[i],&Basic[i]);
  ...
  if (Device.adr_icp[number]>0)
  {
    ...
    switch (evt)
    {
      ...
      case 11:bufs[0]=Key_usa;bufs[3]=Key_3;pool=14;evt=10;
	      ...
	      bufs[6]=hex_to_ascii[icp_wr[0] & Key_mask];count=7;
      break;
    }  
    if (count > 0)
    {
...
/*********** обработка сервисного запроса и выдача ответа **********/
void CommunnicLink (unsigned char buf[],unsigned char num_port)
{
  ...
  if (k == 0) /*нет ошибки циклического кода */
  {
	...
	if (buf[7]>0)
	{
      ...
      case 11:if (buf[7]< Max_icp)
	      { /*запись в модуль новой конфигурации или калибровки*/
      ...
      case 12:ReadFromArchive(buf);break;/*чтение архива*/
      ...
      case 20:if (buf[7] < Max_pnt) /*просмотр переменных базовых точек*/
	      {
      ...
      case 28:ReadFromMinArch(buf);break;/*читать минутный рпхив*/
...
/*************** настройки конфигурации для терминала *********************/
void ConfigSetToMMI (unsigned char config[],unsigned char select[])
{
  unsigned char i,j,k;
...
/********* сохранение переменных процесса точки учёта в ЭНП************/
void SaveParameters (unsigned char num_pnt,struct dynparams bs)
{
  ...
  } /* сохранение счётчиков усреднений */
  for (i=0;i<4;i++)
  {
...
void SendToFlowMeter (unsigned char num)
{
  ...
  switch (evt_meter[num])
  {
    case 0:data[1]=3;data[2]=0;data[3]=244;data[4]=0;data[5]=10;
	   ...
	   break;/*? Їа?б бв вгб  Ё ¤ ??ле*/
    case 1:data[1]=3;data[2]=1;data[3]=2;data[4]=0;data[5]=4;
	   typ_pool=2;evt_meter[num]=0;meter_func=3;break;/*? Їа?б в?в ? ???а?ў*/
    case 2:data[1]=5;data[2]=0;data[3]=4;data[4]=255;data[5]=0;
	   typ_pool=3;evt_meter[num]=0;meter_func=5;break;/*гбв ??ўЄ  ?г?п*/
    case 3:data[1]=5;data[2]=0;data[3]=2;data[4]=255;data[5]=0;
	   typ_pool=4;evt_meter[num]=0;meter_func=5;break;/*бЎа?б в?в ? ???а?ў*/
...
/********** преобразование данных от массомера *************/
void DataFromMeter (unsigned char data[],unsigned char count,
		    unsigned char number )
{
...
/***********  передать запрос в ведомый контроллер******************/
void SendToSlaveController (unsigned char evt)
{
  unsigned char i, k, cr[4], buf[256], buf_modbus[240]; float value;
  unsigned long ks;
  buf[0]=1; buf[1]=buf[2]=buf[3]=buf[4]=0;
  switch(evt)
  {
    case 0:pool_slv=1; buf[5]=2; buf[6]=11;break;     /*запрос статуса*/
    case 1:pool_slv=2; buf[5]=3; buf[7]=sec; buf[8]=min; buf[9]=hour;
	   buf[10]=day; buf[11]=month; buf[12]=year/256;
	   buf[13]=year-buf[12]*256; buf[6]=18;break;/*передача даты-врем*/
    case 2:pool_slv=3; buf[5]=5;
	   for (i=0; i<60; i++)
	   {
	     value=Basic[0].dyn[i]; ConvToBynare(value,cr);
	     for (k=0;k< 4;k++) buf[7+k+i*4]=cr[k];
	   } buf[6]=251;break;  /*передача переменных первой точки*/
    case 3:pool_slv=4; buf[5]=4; buf[6]=11;break;
    case 7:pool_slv=8; buf[5]=13; buf[7]=Device.addr;
	   buf[8]=Device.com[3][0]; buf[9]=Device.com[3][1];
	   buf[10]=Device.com[4][0]; buf[11]=Device.com[4][1];
	   buf[12]=Device.com[5][0]; buf[13]=Device.com[5][1];
	   buf[6]=18;break;
    case 8:pool_slv=9; buf[5]=15;
	   X607_ReadFn(0xa000+2*256,240,buf_modbus);
	   for (i=0;i<240;i++) buf[7+i]=buf_modbus[i];buf[6]=251;break;
    case 9:pool_slv=10; buf[5]=17;
	   X607_ReadFn(0xa000+9*256,240,buf_modbus);
	   for (i=0;i<240;i++) buf[7+i]=buf_modbus[i];buf[6]=251;break;
  }
  CalcCRC32(buf,buf[6]-4,&ks); ConvLongToBynare(ks,cr);
  buf[buf[6]-4]=cr[0]; buf[buf[6]-3]=cr[1]; buf[buf[6]-2]=cr[2];
  buf[buf[6]-1]=cr[3];
  ToComBufn_3(buf,buf[6]); WaitTransmitOver(3);
}
/**************************************************/
void ReceiveFromSlaveContr(unsigned char buf[], unsigned char *evt)
{
  unsigned char i, k, j, cr[4]; float value; unsigned long ks;
  if (buf[0] != 1) goto M;
  i=buf[6]-4; CalcCRC32(buf,buf[6]-4,&ks);
  ConvLongToBynare(ks,cr); k=0;
  for (j=0;j<4;j++) if (buf[i+j] != cr[j]) {k=1;break;}
  if (k == 0)
  {
    switch(pool_slv)
    {
      case 1:if (buf[7]==0) *evt=1;else *evt=3;break;
      case 2:*evt=2;break; case 3:*evt=0;break;
      case 4:if (SecurityConvert(buf[7],buf[8],buf[9],buf[10],cr)==0)
	     {
	       value=ConvToFloat(cr[0],cr[1],cr[2],cr[3]);
	       if (value>=0.6 && value<1.3) Basic[0].dyn[29]=value;
	     } *evt=0;break;
      case 8:*evt=8;break; case 9:*evt=9;break; case 10:*evt=0;break;
    }
  }
  M: pool_slv=0;
}
